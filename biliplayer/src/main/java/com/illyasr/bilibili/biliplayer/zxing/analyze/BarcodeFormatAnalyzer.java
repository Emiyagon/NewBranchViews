    ...
    public Result analyze(byte[] data, int dataWidth, int dataHeight,int left,int top,int width,int height) {
        ...
        if(mReader != null){
            try {
                ...
                rawResult = decodeInternal(source,true);
                ...
                if(rawResult == null ){
                    if(rawResult == null ){
                        ...
                        rawResult = decodeInternal(new PlanarYUVLuminanceSource(rotatedData,dataHeight,dataWidth,top,left,height,width,false),true);
                    }

                    if(true){
                        rawResult = decodeInternal(source.invert(),true);
                    }
                }
                if(rawResult != null){
...
//                result = mReader.decode(new BinaryBitmap(new HybridBinarizer(source)),mHints);
                result = mReader.decode(new BinaryBitmap(new HybridBinarizer(source)),null);
            }catch (Exception e){
...
//                result = mReader.decode(new BinaryBitmap(new GlobalHistogramBinarizer(source)),mHints);
                result = mReader.decode(new BinaryBitmap(new GlobalHistogramBinarizer(source)),null);
            }
        }catch (Exception e){
